...
from muesliswap_onchain_governance.offchain.staking.mint_add_vote_permission import (
    main as add_vote_permission,
)
from muesliswap_onchain_governance.offchain.tally.execute_add_vote_permission import (
    main as execute_add_vote_permission,
)
...
def test_delegated_add_vote():
    ...
    vote_permission_tx, vote_permission_cbor = add_vote_permission(
        wallet=DEFAULT_TEST_CONFIG.voter_wallet_name,
        proposal_id=tally_state.params.proposal_id,
        proposal_index=0,
        tally_auth_nft_tk_name=gov_nft_name,
    )
    wait_for_tx(vote_permission_tx)
    tx = execute_add_vote_permission(
        wallet=DEFAULT_TEST_CONFIG.batcher_wallet_name,
        vote_permission_cbor_hex=vote_permission_cbor.hex(),
    )
    wait_for_tx(tx)
